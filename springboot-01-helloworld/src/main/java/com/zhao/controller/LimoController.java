...
public class LimoController {
    ...
    @RequestMapping(value = "/limo/{place}/{id}", produces = "application/json;charset=utf-8")
    public String getLimoDetails(@PathVariable String place, @PathVariable int id) {
        JSONObject jsonObject = new JSONObject();
        ...
        switch (place) {
            case "index":
                ...
                break;
            case "theme":
                jsonObject.put("breadcrumb", "主题房车");
                break;
            case "general":
                jsonObject.put("breadcrumb", "尊享房车");
                break;
            default:
                break;
        }
    ...
    @RequestMapping(value = "/limo/recommend", produces = "application/json;charset=utf-8")
    public String recommend() {
        List<Limo> recommend = limoService.getLimoRecommend();
        JSONArray jsonArray = new JSONArray();
        recommend.forEach(limo -> {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("name", limo.name);
